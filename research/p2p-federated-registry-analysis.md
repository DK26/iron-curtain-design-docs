...
| **Bandwidth limiting per client**           | Kraken `egress_bits_per_sec` / `ingress_bits_per_sec`, IPFS #3065         | IC exposes `max_upload_speed` and `max_download_speed` in `settings.yaml`. Default upload: 1 MB/s. Default download: unlimited. IPFS's 9-year-unresolved bandwidth issue (#3065, 73 👍) proves this must ship from day one.                                                                                                      |
...
| **Size-based piece length**                 | Kraken `PieceLengths` config                                              | IC uses: <5MB → HTTP-only (no pieces), 5–50MB → 256KB pieces, 50–500MB → 1MB pieces, >500MB → 4MB pieces. Configurable in `settings.yaml`.                                                                                                                                                                                      |
