...
| **Graceful degradation**     | Tracking server down → join codes → direct IP. Relay down → mode-specific: ranked voids/degrades (D007), casual may offer unranked continuation | Workshop server down → git-index. P2P unavailable → HTTP fallback. Each failure has a documented fallback                  | Same resilience philosophy. Should share a unified "failure modes" table across all backend services                                                           |
