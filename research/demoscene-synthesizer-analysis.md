# Demoscene Software Synthesizer Analysis — V2, 4klang, Oidos, and Peers
...
Demoscene software synthesizers solve the same problem IC's LLM audio generation pipeline faces: produce complex, high-quality audio from minimal data. A 96KB executable (.kkrieger) contained a full FPS game with ambient music, weapon sounds, and environment audio — all synthesized in real-time from ~2–5 KB of patch data plus a compressed MIDI-like event stream. This analysis extracts the architectural patterns, patch definition formats, SFX synthesis techniques, and size/quality tradeoffs from six major demoscene synths.

**Key takeaway for IC:** Demoscene synth patch data is almost entirely **arrays of 0–127 byte values** (MIDI-style). These are trivially LLM-generatable — a patch is essentially 50–150 numbers in a defined schema. The ABC→MIDI→SoundFont pipeline in `llm-soundtrack-generation-design.md` is the right approach for music, but for procedural SFX, the demoscene pattern of **synthesizer parameter arrays** interpreted by a built-in synth engine would be far more compact and flexible than rendering pre-recorded samples.
...
```
3 Oscillators → 2 Filters (single/serial/parallel routing) → Distortion → DC Filter → Stereo Pan → Channel Bus
```

Per-channel effects chain:
```
DC Filter → Compressor → Bass Boost → Distortion/Chorus (configurable order) → DC Filter → Aux Send A/B
```

Global effects:
```
Aux A → Reverb → Master
Aux B → Modulating Delay → Master
Master → DC Filter → Compressor → Low/High Cut → Output
```

### Oscillator (V2Osc) — 8 Modes
...
### Envelope (V2Env) — ADSSR Model

Not standard ADSR — uses an ADSSR (Attack, Decay, Sustain, Sustain-Rate, Release) model:

- **Attack:** Additive ramp toward level 128
- **Decay:** Multiplicative decay toward sustain level
- **Sustain:** Multiplicative decay toward a sustain target (not static — continues decaying)
- **Release:** Multiplicative decay toward zero

States: `OFF → ATTACK → DECAY → SUSTAIN → RELEASE`. 2 envelopes per voice (`NENV=2`). Envelope 0 directly controls voice amplitude.

### Filter (V2Flt) — 8 Modes

| Mode                            | Type                                           |
| ------------------------------- | ---------------------------------------------- |
| BYPASS                          | No filtering                                   |
| LOW / BAND / HIGH / NOTCH / ALL | State-variable filter (V2LRC), 2× oversampled |
| MOOGL                           | Moog-style lowpass (4-pole ladder)             |
| MOOGH                           | Moog-style highpass                            |

2 filters per voice (`NFLT=2`). Three routing modes: `SINGLE` (one filter), `SERIAL` (filter1 → filter2), `PARALLEL` (mix with balance control). Parameters: `mode`, `cutoff`, `reso` (resonance).

### LFO (V2LFO) — 5 Waveforms
...
- **V2Reverb:** Freeverb-style — 4 parallel comb filters per stereo channel (with damping/feedback), fed into 2 serial allpass filters, plus a high-pass low-cut filter. Fixed delay line sizes (1309, 1635, 1811, 1926 samples for left combs). Parameters: `revtime`, `highcut`, `lowcut`, `vol`.
...
```c
struct V2Sound {
    sU8 voice[sizeof(syVV2)/sizeof(sF32)];  // ~50-60 bytes — one byte per float parameter
    sU8 chan[sizeof(syVChan)/sizeof(sF32)];  // ~30-40 bytes — channel effect parameters
...
**All parameters are single bytes (0–127 range).** The synthesis engine linearly maps each byte to its float working range. A typical patch is **80–150 bytes total** depending on modulation complexity.
...
- `notenum/noteptr` — Note on/off events with delta timing
- `pcnum/pcptr` — Program changes
- `pbnum/pbptr` — Pitch bend events
- 7 CC tracks: `ccnum[7]/ccptr[7]` — Controller changes
...
- **CC 1–7:** Stored in `chans[].ctl[]` for modulation matrix sources
...
- **System Reset:** Panic — kills everything

### .kkrieger Audio Implementation

From the `werkkzeug3_kkrieger/` directory in fr_public, the file `_viruz2.hpp` shows a `sViruz2` class wrapping `CV2MPlayer`. The entire game's audio — ambient music, weapon sounds, explosions, UI feedback — was synthesized by V2 from:

- **Patch data:** ~128 patches × ~100 bytes average = ~12 KB uncompressed (compresses to ~2–5 KB)
- **Song/event data:** V2M stream with note events for music tracks, likely ~5–15 KB compressed
- **Synth engine code:** ~15–20 KB compressed (the V2 ASM core was extremely compact)

Total audio footprint in the 96 KB executable: estimated **8–15 KB** including engine code, patches, and event data. This produced:
...
- **Arithmetic node:** Stack operations — push, pop, add, multiply, etc.
...
This stack-based approach is extremely compact in compiled form — each instrument is a sequence of node operations. The NASM assembly core evaluates the stack machine per sample.

### Key Characteristics

- **16 MIDI channels** via a single VST instance (singleton pattern — same as V2)
...
- **Polyphony:** Configurable per patch (1×, 2×, etc.)
...
- Synth engine (player): ~1–2 KB compressed
- Patch definitions: ~200–800 bytes for 4–8 instruments
- Song data: ~500–2,000 bytes
- Total audio: ~2–3 KB in a 4 KB intro

### Sound Design Notes (from pouet.net community)

- Stack-based approach described as "a coder's synth" — powerful but requires understanding signal flow
- Wobble bass achievable via LFO → filter cutoff modulation (stored parameters allow cross-instrument modulation)
- The 3.11 "8klang" extension added stereo flags, 64 slots per instrument, and instrument linking — aimed at 8K+ intros
...
### Architecture — Pure Additive Synthesis

Oidos takes a radically different approach from subtractive synths like V2 or 4klang. It is a **pure additive synthesizer** — sound is generated by summing a large number of individual sine waves ("partials"). No filters, no oscillator waveform selection.

The synthesis process:
1. From a **random seed**, generate `modes × fat` partials with frequencies distributed around the base note
...
| `q*` parameters          | Quantization — rounds internal values for better compression |

### Reverb (OidosReverb)

A separate VST effect. "Strength in numbers" approach — many parallel filtered feedback delays. Parameters: `mix`, `pan`, `delaymin`/`delaymax`/`delayadd`, `halftime`, filter limits, `dampen` limits, `n` (number of delays), `seed`.
...
- Playback is just mixing precomputed buffers — very cheap at runtime
- **Parameter automation is impossible** (sound is fully determined at note-on time)
- Memory cost = (number of unique tones) × (longest note length)
...
`github.com/gopher-atz/64klang`. C++ synth core (SSE4.1), .NET/WPF GUI. MIT license. 283 GitHub stars. By Dominik Ries (gopher/Alcatraz) — same author as 4klang.

### Architecture — Modular Node Graph

64klang is a **modular, node-graph-based synthesizer** — the "big brother" of 4klang, designed for **64 KB intros**. Unlike 4klang's stack machine, 64klang uses a fully general directed graph of processing nodes.

Key architectural features:
- **Node graph evaluated per sample** — enables sample-exact feedback loops
- **Unlimited connection topology** — any node output can feed any node input
...
- **C++ core with heavy SSE4.1 optimization** — inspired by Ralph Borson (revivalizer)'s blog posts on SIMD softsynth development
- **16 MIDI channels** via singleton VST instance
- **Full source code available** (VSTiPluginSourceCode directory)

### History

- v1 (~2010–2011): Extended 4klang concept, ASM core, Win32 GUI — functional but unmaintainable
- v2 (2012–2014): Complete rewrite, C++/SSE4.1 core, .NET WPF GUI with proper node graph visualization, zooming, etc.
...
Distributed as ZIP from crinkler.net. x86 NASM assembly player + Renoise VST. By Aske Simon Christensen (Blueberry) — same author as Oidos. Released 2013.

### Architecture — 2-Operator Phase Modulation
...
- **2× oversampling:** Reduces aliasing artifacts
- **Careful use of the two PM operators** with various waveforms

### Waveforms

Available as BaseWave and ModWave:
- Sine, triangle, sawtooth, square, parabola (asymmetric — primarily useful as ModWave)

### Workflow

Same as Oidos — compose in Renoise, convert `.xrns` to assembly:
```
Renoise (.xrns) → ClinksterConvert → clinkster.asm → link with intro
```
...
- **Extremely compact player:** The assembly player is tiny — well-suited for 4K intros
...
- **Musician-friendly:** Blueberry describes it as "made for musicians who are bad coders, rather than coders who are bad musicians" — contrasting with 4klang's coder-oriented stack model
- **Multithreaded version available:** Computes left/right channels in separate threads for ~2× faster precomputation
...
The key innovation is the **DAW conversion pipeline** — compose normally in a standard DAW using WaveSabre's VSTs, then convert the entire project to a minimal binary representation for playback in the intro.
...
Tunefish is a **virtual analog synthesizer** — a more traditional subtractive synthesizer architecture:
...
| --------- | ----------------------------------- | ---------------------------------------------------------------------------------- |
| V2        | Subtractive (osc → filter → fx) | Full-featured, versatile, proven in production game (.kkrieger)                    |
...
| --------- | --------------------------------------------- | --------------------------- | -------------------------------------------------------- |
| V2        | Byte array (0–127) + 3-byte mod entries     | 80–150 bytes              | **Excellent** — just 50–150 integers in a schema     |
| 4klang    | Stack operation sequence with byte parameters | 50–200 bytes              | **Good** — ordered node list with parameters           |
| Oidos     | ~20 float parameters (quantized)              | ~80 bytes                   | **Excellent** — small parameter count, well-documented |
| 64klang   | Node graph serialization                      | Variable (graph complexity) | **Moderate** — graph topology adds complexity          |
| Clinkster | ~10–15 parameters per instrument            | ~60 bytes                   | **Excellent** — minimal parameter count                |
| WaveSabre | Per-VST parameter blobs                       | Variable                    | **Moderate** — depends on which synth VSTs used        |

### Size Comparison (Typical Audio Budget)

| Context              | Synth Engine         | Patch Data    | Song Data     | Total     |
| -------------------- | -------------------- | ------------- | ------------- | --------- |
| 4K intro             | 4klang/Clinkster     | 200–800 B   | 500–2,000 B | 1–3 KB  |
| 8K intro             | 4klang 8klang/Oidos  | 500–2,000 B | 1–5 KB      | 2–6 KB  |
| 64K intro            | V2/64klang/WaveSabre | 2–8 KB      | 5–20 KB     | 8–25 KB |
| 96K game (.kkrieger) | V2                   | ~5 KB         | ~8 KB         | ~15 KB    |

### Quality Assessment

**V2:** Professional-quality sound — used in actual game production. The combination of bandlimited oscillators, Moog filter emulation, Freeverb reverb, and comprehensive modulation matrix produces sounds indistinguishable from commercial synths for most use cases. The subtractive model is well-understood and covers the vast majority of classic game audio needs.
...
**Oidos:** Unique and often wonderful-sounding — the additive approach excels at evolving pad textures, bell-like tones, and sounds with unusual harmonic content that subtractive synths struggle with. However, it's computationally heavy (many partials to sum) and the precomputation model means no real-time parameter changes.

**64klang:** The most sonically versatile — physical modeling, cross-modulation, arbitrary feedback loops. Can produce sounds none of the others can. The per-sample node graph evaluation enables sample-accurate feedback for Karplus-Strong plucked strings, waveguide wind instruments, etc.

**Clinkster:** Surprisingly good sound from just 2-op PM synthesis. The layering/detuning/stereo-spread trick compensates for the lack of filters. Excel at synthetic/digital timbres — less suitable for warm analog-style sounds.

**WaveSabre:** High production values — Logicoma's intros consistently have excellent audio quality.
...
- Minimal reverb → dry, immediate feedback

**Footsteps/Impacts:**
- Very short noise burst with tight bandpass filter
- Envelope: near-zero attack, 10–30ms decay
- Filter cutoff variation between hits for natural variation
- Subtle pitch randomization via modulation matrix (velocity → pitch)

### General Demoscene SFX Patterns

The demoscene SFX approach can be summarized as:

1. **Every sound is a synthesizer event** — no samples, no recordings
...
| ------------- | -------------------------------------------- | -------------------------------------------------- |
| **Oidos**     | 33.2% Rust — synth core and reverb modules | Active, modern Rust edition, proven in demoscene   |
| **sonant-rs** | Rust port of Sonant 4K synthesizer           | Directly relevant — demoscene synth in pure Rust |
...
For IC's purposes, the ABC→MIDI→SoundFont pipeline (documented in `llm-soundtrack-generation-design.md`) handles music generation well. For procedural SFX, a lightweight **parameter-driven synth** (inspired by V2's byte-array patch format) could be ~500–1,000 lines of Rust — significantly simpler than porting a full demoscene synth, and tailored to IC's specific SFX needs.
...
The demoscene analysis **validates** the ABC→MIDI→SoundFont approach documented in `llm-soundtrack-generation-design.md`:

1. **MIDI-like event streams are the proven pattern** — V2M, 4klang export, Oidos converter all produce compressed MIDI-style event data
2. **Symbolic music representation is compressible** — delta encoding + entropy coding achieves excellent ratios (the demoscene proves this)
3. **LLMs can generate patch parameters** — if a human musician can create a V2 patch by setting 50–150 byte values in a GUI, an LLM can generate equivalent JSON: `{"oscillator": "noise", "filter_cutoff": 100, "decay": 90, ...}`

### For Procedural SFX (New Design Consideration)

The demoscene pattern suggests a possible IC extension (Phase 7+, optional):

**A lightweight SFX synthesis engine** in `ic-audio` that generates sound effects from YAML-defined parameter schemas — no sample files needed. A mod could define:
...
This is strictly a future proposal, not a current commitment. But the demoscene proves the approach works — .kkrieger shipped an entire game's audio this way.

### For Asset Size Optimization

The demoscene size discipline offers useful principles:
- **Quantize everything** — Oidos's quantization parameters reduce entropy before compression
- **Delta-encode event streams** — Timestamps as deltas from previous event, not absolute
- **Reuse through variation** — One patch + modulation matrix beat five separate patches
- **Precompute when possible** — Trade load-time computation for storage (Oidos/Clinkster model)

### For LLM Patch Generation

The V2 and Oidos patch formats are **ideal for LLM generation**:
- Small, bounded parameter spaces (20–60 parameters, each 0–127 or a small float range)
- Well-defined parameter semantics (each parameter has a clear audio effect)
- Deterministic — same parameters always produce the same sound
- JSON-representable — `{"modes": 15, "fat": 4, "harmonicity": 80, "decay_low": 40, ...}`
...
LLM generates: {"type": "laser", "osc": "sin+pulse", "filter": "bandpass_sweep_down",
                 "attack_ms": 2, "decay_ms": 150, "pitch": "C5→C3", "reverb": 0.1}
Engine renders: 150ms WAV from parameters
```

This is documented here as a research finding, not a committed design. The ABC→MIDI→SoundFont pipeline remains IC's planned approach for Phase 7.
