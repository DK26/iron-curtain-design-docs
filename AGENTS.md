...
| D034 | SQLite as embedded storage (services + client)    | `rusqlite` for all persistent state: relay match history, workshop registry, matchmaking ratings, client replay/save/asset indices, gameplay event logs; preserves "just a binary" deployment; no external DB; FTS5 for search; WAL for concurrent reads; WASM-compatible; ad-hoc SQL investigation without OTEL stack |
