...
**Converter tool:** The `cnc-formats` CLI includes a `convert` subcommand (behind the `miniyaml` feature flag) that translates existing OpenRA MiniYAML mod data to standard YAML on disk. Available for permanent, clean migration. The same CLI also provides `validate` (structural correctness check) and `inspect` (dump archive contents, frame counts, palette info) for all supported C&C formats.
